            ...
            if self._measureCnt == WEIGHT_SAMPLES:
                ...
                if (self._changeDetected): 
                    if abs(self._weightDiff) < .1:
                        ...
                        self.streamer.log(WIIBOARD_NAME + "_weight", self._weight)
                        self.streamer.log(WIIBOARD_NAME + "_status", "Weight stabilized") 

                        if ((self._weight - self._lastWeightSent) < 0) and (self._lastWeightSent != 0):
                            self.streamer.log(WIIBOARD_NAME + "_decremented_weight", abs(self._weight - self._lastWeightSent)) 
                        self.streamer.flush()
                        self._changeDetected = False
                ...
                if abs(self._weightDiff) > .3:
                    ...
                    self.streamer.log(WIIBOARD_NAME + "_status", "Change detected ...")
                    self.streamer.flush()
                    self._lastMeasurement = self._weight
